...
public class l06hasPath {
    ...
    public static boolean hasPath(ArrayList<Edge> graph[], int src, int dest, boolean vis[]){  // TC -> O(v+e)
        if(src == dest) return true; // check if we are at dest
        vis[src] = true;

        for(int i =0; i<graph[src].size(); i++){
            Edge e = graph[src].get(i);

            if(!vis[e.dest] && hasPath(graph,e.dest/*i.e.neighbour*/,dest, vis)){
                return true;
            }
        }

        return false; // in case of no path
    }
