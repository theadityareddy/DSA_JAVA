...
public class l01scc_kosarajuAlgo {
    ...
    public static void kosaraju(ArrayList<Edge> graph[], int V){  //T.C  = O(V+E)
        ...
        //step 1 : adding all our nodes in stack to have a sequence for SCC
        ...
        for(int i = 0; i<V; i++){ //copying the graph but with reverse edges
            for(int j = 0; j<graph[i].size(); j++){
                ...
                tranpose[e.dest].add( new Edge(e.dest,e.src)); //reverse edge : e.dest -> e.src
            }
        }

        //Step 3 : DFS acc to stack on transpose graph
        while(!s.isEmpty()){
            int curr = s.pop();
            if(!vis[curr]){
